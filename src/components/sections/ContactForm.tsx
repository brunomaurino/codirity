...
export function ContactForm() {
 ...
 return (
 <div
 className={cn("relative overflow-hidden","bg-white border border-[var(--border)] rounded-2xl","p-8 md:p-10 shadow-lg","before:absolute before:top-0 before:left-0 before:right-0 before:h-1","before:bg-gradient-to-r before:from-brand-dark before:via-brand before:to-brand-light")}
 >
 {/* Header */}
 <div className="mb-8">
 ...
 this bundle: this sub-copy and the privacy note below both moved off`text-gray-500`, which measures 4.47:1 on the white card — under AA
 for 14px text. */}
 ...
 <Input
 label="First Name *"name="firstName"placeholder="John"value={formData.firstName}
 onChange={handleChange}
 required
 disabled={isSubmitting}
 />
 <Input
 label="Last Name *"name="lastName"placeholder="Doe"value={formData.lastName}
 onChange={handleChange}
 required
 disabled={isSubmitting}
 />
 ...
 {/* Email */}
 <Input
 label="Email Address *"name="email"type="email"placeholder="john@company.com"value={formData.email}
 onChange={handleChange}
 required
 disabled={isSubmitting}
 />
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
 <Input
 label="Company Name"name="company"placeholder="Your Company"value={formData.company}
 onChange={handleChange}
 disabled={isSubmitting}
 />
 <Select
 label="Service Interested In"name="service"placeholder="Select a service"options={serviceOptions}
 ...
 <textarea
 id="message"name="message"placeholder="Tell us about your project, challenges, or goals..."value={formData.message}
 onChange={handleChange}
 required
 disabled={isSubmitting}
 className={cn("w-full min-h-[130px] resize-y","bg-white border border-gray-200 rounded-xl","px-4 py-3 text-sm text-gray-900","placeholder:text-gray-400","transition-all duration-300","focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand","disabled:opacity-60 disabled:cursor-not-allowed")}
 />
 </div>
